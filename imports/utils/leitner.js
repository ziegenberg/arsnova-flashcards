...
export let LeitnerUtilities = class LeitnerUtilities {
	...
	static selectCardsByOrder (cardset, boxActiveCardCap, algorithm, user) {
		...
		for (let l = 0; l < algorithm.length; l++) {
			...
			let filter = MiscUtilities.getUniqData(leitnerCards, 'card_id');
			let priorities = MiscUtilities.getUniqData(leitnerCards, 'priority');
	...
	static getCardsetInfo (cardset) {
		return [
			...
			[TAPi18n.__('cardset.info.author', {}, ServerStyle.getClientLanguage()), Profile.getAuthorName(cardset.owner)],
	...
	static getCurrentMaxBonusPoints (cardset) {
		if (cardset.workload.bonus.maxPoints === undefined) {
			return bonusFormConfig.defaultMaxBonusPoints;
		} else {
	...
	static getCurrentMinLearned (cardset) {
		if (cardset.workload.bonus.minLearned === undefined) {
			return bonusFormConfig.defaultMinLearned;
		} else {
